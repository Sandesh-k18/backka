...
import { Loader2, RefreshCcw, Trash2, ShieldCheck, ExternalLink } from "lucide-react";
...
const UserDashboard = () => {
  ...
  const profileUrl = session?.user
    ? `${baseUrl}/u/${(session.user as SessionUser).username?.toLowerCase()}`
    : "";
  ...
  return (
    <div className="container mx-auto my-8 p-4 md:p-6 bg-white rounded shadow-sm max-w-6xl min-h-[80vh]">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl md:text-4xl font-bold text-gray-900">User Dashboard</h1>

        {/* REPLACED DIALOG WITH SIMPLE NAVIGATION BUTTON */}
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
          <input
            ...
            value={profileUrl}
            readOnly
            className="input input-bordered flex-grow p-2 bg-gray-50 rounded border text-sm overflow-hidden text-ellipsis cursor-default"
          />
          <Button onClick={copyToClipboard} className="shrink-0 bg-indigo-600 hover:bg-indigo-700 text-white">
            Copy
          </Button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCcw className="h-4 w-4 text-gray-600" />}
          </Button>

          {messages.length > 0 && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="destructive" disabled={isDeletingAll} className="flex-1 sm:flex-none h-10 font-bold text-xs uppercase px-6">
              ...
              <AlertDialogContent className="rounded-2xl">
                ...
                <AlertDialogFooter className="gap-2">
                  ...
                  <AlertDialogAction onClick={handleClearAll} className="bg-red-600 hover:bg-red-700 rounded-full text-white">
                    Delete All
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
